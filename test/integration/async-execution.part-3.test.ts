...
`);
	...
	for (const extraFile of [false, true]) {
		it(`structured review preserves its verdict and reports a missing extra file (${extraFile})`, { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
			const review = { verdict: "changes_required", findings: ["Add the missing guard"] };
			mockPi.onCall({ output: "", structuredOutput: review });
			const id = `async-review-contract-${extraFile}-${Date.now().toString(36)}`;
			const outputPath = path.join(tempDir, "missing-review-report.md");
			executeAsyncSingle(id, {
				agent: "reviewer",
				task: "Review the code without modifying files; return the structured verdict.",
				agentConfig: makeAgent("reviewer", { tools: ["read"], completionGuard: false }),
				ctx: { pi: { events: { emit() {} } }, cwd: tempDir, currentSessionId: "session-1" },
				artifactConfig: { enabled: false, includeInput: false, includeOutput: false, includeJsonl: false, includeMetadata: false, cleanupDays: 7 },
				shareEnabled: false,
				sessionRoot: path.join(tempDir, "sessions"),
				maxSubagentDepth: 2,
				acceptance: false,
				structuredOutputSchema: { type: "object", required: ["verdict", "findings"], properties: { verdict: { enum: ["approved", "changes_required"] }, findings: { type: "array", items: { type: "string" } } } },
				...(extraFile ? { output: outputPath, outputMode: "file-only" as const } : {}),
			});
			const payload = JSON.parse(fs.readFileSync(await waitForAsyncResultFile(id, 15_000), "utf8")) as AsyncResultPayload;
			assert.equal(payload.success, !extraFile);
			assert.equal(payload.exitCode, extraFile ? 1 : 0);
			assert.deepEqual(payload.results[0]?.structuredOutput, review);
			assert.equal(fs.existsSync(outputPath), false);
			if (extraFile) {
				assert.match(payload.results[0]?.error ?? "", /Required file-only output was not produced/);
				assert.match(payload.results[0]?.error ?? "", /Output contract not fulfilled; structured result exists\./);
				const status = await waitForAsyncState(id, (candidate) => candidate.state !== "running");
				assert.match(status.steps?.[0]?.error ?? "", /structured result exists/);
			} else {
				assert.equal(payload.results[0]?.error, undefined);
			}
		});
