...
describe("formatWindowsExecutionHint", () => {
	it("is empty outside Windows", () => {
		assert.equal(formatWindowsExecutionHint("/home/user/proj", "linux"), "");
		assert.equal(formatWindowsExecutionHint("/home/user/proj", "darwin"), "");
	});

	it("names only verified paths on Windows", () => {
		const emptyCwd = fs.mkdtempSync(path.join(os.tmpdir(), "pos-winctx-"));
		try {
			const hint = formatWindowsExecutionHint(emptyCwd, "win32");
			assert.match(hint, /Windows execution context \(verified on this machine\):/);
			assert.ok(hint.includes(process.execPath));
			assert.ok(hint.includes(os.tmpdir()));
			assert.match(hint, /Git Bash consumes and rewrites unescaped backslashes/);
			assert.match(hint, /MSYS paths such as \/tmp\/\.\.\.| \/c\/Users\/\.\.\./);
			assert.doesNotMatch(hint, /A project interpreter exists/);
		} finally {
			fs.rmSync(emptyCwd, { recursive: true, force: true });
		}
	});

	it("advertises a project interpreter only when it exists", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pos-winctx-venv-"));
		try {
			assert.doesNotMatch(formatWindowsExecutionHint(cwd, "win32"), /A project interpreter exists/);
			const venvPython = path.join(cwd, ".venv", "Scripts", "python.exe");
			fs.mkdirSync(path.dirname(venvPython), { recursive: true });
			fs.writeFileSync(venvPython, "stub");
			const hint = formatWindowsExecutionHint(cwd, "win32");
			assert.match(hint, /A project interpreter exists at /);
			assert.ok(hint.includes(venvPython));
		} finally {
...
describe("child system prompt windows hint injection", () => {
	it("appends the hint exactly once for child system prompts", async () => {
		let beforeAgentStart: ((event: { systemPrompt: string }) => { systemPrompt: string } | undefined) | undefined;
		registerSubagentPromptRuntime({
			on(event: string, handler: (payload: { systemPrompt: string }, ctx?: unknown) => { systemPrompt: string } | undefined) {
				if (event === "before_agent_start") beforeAgentStart = handler;
			},
