...
import "pi-orca-subagents";
import type { AgentToolResult } from "@earendil-works/pi-agent-core";
import { registerWorkflowResource, type RegisterWorkflowResourceInput, type WorkflowResourceDefinition, type WorkflowResourceRegistration } from "pi-orca-subagents/workflow-resources";
...
`, "utf-8");
		fs.writeFileSync(path.join(consumerRoot, "tsconfig.json"), JSON.stringify({
			compilerOptions: {
				...
				paths: {
					"pi-orca-subagents": [path.join(projectRoot, "index.ts")],
					"pi-orca-subagents/workflow-resources": [path.join(projectRoot, "src/api/workflow-resources.ts")],
...
test("published extension APIs use supported package entrypoints", async () => {
	...
	assert.match(fs.readFileSync(path.join(projectRoot, "native.ts"), "utf8"), /await import\("\.\/src\/extension\/index\.ts"\)/);
	...
	assert.deepEqual(packageJson.exports, {
        ...
        "./child-execution": "./src/api/child-execution.ts",
	});
	const agents = await import("pi-orca-subagents/agents");
	...
	const backgroundWork = await import("pi-orca-subagents/background-work");
	...
	const externalJobProvider = await import("pi-orca-subagents/external-job-provider");
	...
	const externalRuns = await import("pi-orca-subagents/external-runs");
	...
	const capability = await import("pi-orca-subagents/capability-ceiling");
	const workflowResources = await import("pi-orca-subagents/workflow-resources");
	...
	const delegation = await import("pi-orca-subagents/delegation");
	assert.equal(delegation.SUBAGENT_DELEGATION_REQUEST_EVENT, "prompt-template:subagent:request");
	const preflight = await import("pi-orca-subagents/preflight");
	...
	const controlChannel = await import("pi-orca-subagents/control-channel");
	assert.equal(typeof controlChannel.requestAsyncStop, "function");
	const intercomBridge = await import("pi-orca-subagents/intercom-bridge");
	assert.equal(typeof intercomBridge.resolveIntercomSessionTarget, "function");
	const childToolPlan = await import("pi-orca-subagents/child-tool-plan");
	...
	const sharedTypes = await import("pi-orca-subagents/shared-types");
	...
	const projectPanes = await import("pi-orca-subagents/project-panes");
...
test("host-owned development packages use the supported SDK baseline", () => {
	...
	assert.equal(
		...
		"0.85.1",
		"POS acceptance uses the real SDK; upstream harness isolates its runtime shim",
	);
});
